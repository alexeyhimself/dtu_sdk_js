...
const REAL_BACKEND_OPERATION = false; // default is true, but false for ci and online demos
...
async function DTU_RX_API_submint_report(report, api_url) { // https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch
  //console.log(Base64.encode(JSON.stringify(report)))
  const response = await fetch(api_url + '/api/submit', { // default options are marked with *
    ...
    // for passing through application/x-www-form-urlencoded which is used instead of application/json due to no-cors header
    body: Base64.encode(JSON.stringify(report)), // https://stackoverflow.com/questions/41219542/how-to-import-js-modules-into-typescript-file
  });
...
const DEFAULT_TOPIC = 'default'; // window.location.hostname; -- a good value to send as well
